...
import com.github.pk65.tools.LinesProcessor
import scala.annotation.tailrec

class LinesProcessorSuite extends FunSuite {
  val expectedText = """Oct 03 09:11:20 service9 postfix/cleanup[1221726]: 54BE7192007D: warning: header Subject: [Service 9] Report acceptance request for period 2024-10 from localhost[127.0.0.1]; from=<service9@example.com> to=<USER1@EXAMPLE.COM> proto=ESMTP helo=<service9>
    ...
    |Oct 03 09:11:20 service9 postfix/qmgr[1065995]: 54BE7192007D: removed""".stripMargin
  ...
  test("add lines one by one to the storage") {
    ...
    assertEquals(readLines(stream.getLines(), LinesStorage("user1@example.com")).mkString("\n"), expectedText)
  }

  @tailrec
  private def readLines(src: Iterator[String], storage: LinesStorage, filteredLines: List[String] = List.empty): List[String] = {
    ...
    line match {
      case None => lines
      case Some(l) => readLines(src, LinesProcessor.filterLines(storage.copy(newLine = Some(l))), lines)
    }
  }
}